...
4.  **Create Table:** Inside the constructor, after the SQS queue definition, add the code to create the DynamoDB table.
    ```typescript
      ...
      new cdk.CfnOutput(this, 'ResultsTableName', { value: this.table.tableName });
    ```
...
Update `CoreStack` to use the `NodejsFunction` construct and point it to the correct handler entry path.
    ...
    ```typescript
      ...
      new cdk.CustomResource(this, 'DDBSeedResource', {
        ...
        properties: {
          ...
          Timestamp: Date.now().toString() // Ensures update on change
        }
      });
    ```
...
EOF`, // Use the UPDATED pollingScriptTemplate
          ...
          //'touch /home/ec2-user/textract_error.log && chown ec2-user:ec2-user /home/ec2-user/textract_error.log', // Add textract log later
